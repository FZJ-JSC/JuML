...
namespace juml {
    ...
    void KMeans::initialize_random_centroids(const Dataset& dataset) {
        ...
        std::uniform_int_distribution<intl> index_selector(0, dataset.global_n_features() - 1);
    ...
    void KMeans::initialize_kpp_centroids(const Dataset& dataset) {
        ...
        intl pick_boundary = dataset.global_n_features() - 1;
    ...
    void KMeans::cluster(const Dataset& dataset) {
        ...
        uintl threshold = static_cast<uintl>(std::floor(this->tolerance_ * dataset.global_n_features()));
